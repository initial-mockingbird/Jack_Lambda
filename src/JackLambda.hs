module JackLambda (main',menuCarga) where
...
jugarRonda datos = do
  ...
  putStrLn $ nombre datos' ++ ", Esta es mi primera carta: " ++ take 2 (show manoLambda)
  -- Seleccion de mazo.
  ...
  if blackJack manoPlayer then ganaPlayerBlackJack datos'  else putStrLn (showMano manoPlayer Player (nombre datos') ) >>
...
ganaLambda datos manoPlayer manoLambda = do
  ...
  if dinero datos' < apuesta datos' then (putStrLn ( (show . nombre) datos' ++ ", no te queda dinero. Es el fin del juego para ti.")) >> return ()
    else menuJuego datos'
  return ()
...
ganaPlayer datos = do
  ...
  if dinero datos' > objetivo datos' then
    (putStrLn ( "Felicidades " ++ (show . nombre) datos' ++ ", me has derrotado. Es el fin del juego para mi.")) >> return ()
    else menuJuego datos'
  return ()
...
guardarPartida datos = do
  let file =  "./saved_games/saved_games.txt"
  savedContents <- readFile file
  let contentsByBlocks = map ("{"++) $ splitBy "{" savedContents
  let parsedDatos = show datos
  let newContents = parsedDatos : [x | x<- contentsByBlocks, x=/= parsedDatos]
  appendFile file newContents
  return ()
...
instance Eq GameState where
  (==) datos1 datos2 = nombre datos1 == nombre datos2

instance Show GameState where
  show (GS {juegos,victorias,nombre,gen,dinero,objetivo,apuesta}) =
    "{\n" ++ "juegosJugados::"show juegos ++ "\n" ++ "victoriasLambda::" ++ show victorias ++ "\n" ++
      "nombre::" ++ nombre  ++ "\n" ++ "generador::" ++ ((splitBy " " (show gen))!!5) ++ "\n" ++  "dinero::" ++ show dinero ++ "\n" ++
      "objetivo::"++ show objetivo ++ "\n" ++ "apuesta::" ++ show apuesta ++ "\n" ++ "}"
