...
newtype CartaNumerica = CartaNumerica {n :: Int} deriving(Eq,Show,Ord)
...
instance Enum Rango where
  ...
  fromEnum (L l) = 10 + fromEnum l
  toEnum n | 1<=n && n<=10 = N $ toEnum n
           | 11<=n && n<=13 || n==0 = L $ toEnum (n `mod` 10)
           | otherwise = error $ show n  ++ "Es muy grande"
  
instance Enum Carta where
  succ (Carta valor pal)   | fromEnum  valor == 13 && not (pal == Corazones) = Carta {rango = toEnum 1, palo = succ pal}
                           | fromEnum valor == 13 &&  (pal == Corazones) =  error $ show (Carta{rango=valor,palo=pal})  ++ "Es muy grande"
                           | otherwise =  Carta {rango = succ valor , palo = pal}
  fromEnum (Carta valor pal) = fromEnum valor + 13*(fromEnum pal)
  toEnum x = Carta {rango= toEnum 1,palo = Treboles}
