...
getDineroRonda dineroInicial = do
  ...
  if (read dineroRonda) :: Int < dineroInicial then putStrLn "La apuesta  debe ser mayor o igual que la que se tiene, intente nuevamente" >>
...
hit datos mazo manoPlayer manoLambda = do
  ...
  Just (mazo'',manoPlayer') <- robar mazo' manoPlayer eleccion
...
doubleDown datos mazo manoPlayer manoLambda = do
  let datos' = datos {dinero = ((-) <$> dinero <*> apuesta) datos, apuesta = (*2) $ apuesta dinero }
  let (mazo', manoPlayer') = drawingPhasePlayer datos mazo manoPlayer manoLambda
  if valor manoPlayer' > 21 then ganaLambda datos manoPlayer' manoLambda else showMano manoPlayer' >>
    stand datos' mazo' manoPlayer' manoLambda
  return ()
...
drawingPhasePlayer ::  GameState -> Mazo -> Mano -> Mano -> (Mazo,Mano)
